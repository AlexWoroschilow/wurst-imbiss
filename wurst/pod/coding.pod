...
All the files have lines like this:
      ...
      "$Id: coding.pod,v 1.2 2013/08/30 09:40:03 torda Exp $";
...
Use old fashioned comment markers /* ...*/ not the C++ style. There are two reasons

=over

=item * Homogeneity. We want the code to look the same as the older code.

=item * Portability

The code compiles with the older C standards so it will run on as many
machines as possible. It does not require a new compiler. We can turn
on compiler warnings for newer features. If you use //style comments,
we end up with a soup of warnings from gcc.
