...
double*
dbfreq(void *space, IntSequence **set, Uint setsize, FAlphabet *alpha, double norm) {
	Uint i, j, l, sum = 0;
	double *f;
...
double *
scorefreq(void *space, double *scr, Uint noofscr, FAlphabet *alpha, double *sf, double *dbf) {
	Uint i;
	double sum = 0;
	double *f;
...
double checklambda(double *scr, Uint noofscores, double* sf, double avg, double e_lamb) {
	Uint i;
	double val = 0;
...
double score_evd(double lambda, void* info) {
	Uint i;
	double sum = 0;
...
double relentropy(void *space, Uint *sort, double *scr, Uint noofscr, double *q, double lambda) {
	double sum = 0, av, sumval, K, Ktmp;
...
//	P = malloc(sizeof(double)* (size_t)fix);
	...
	for (*P = 1.0, sum = 1.0, j = 1; j <= 1500 && sum > 0.0000001; sumval += sum /= j++) {

		first = last = range;
		for (ptrP = P + ((hi += scr2[noofscr - 1]) - (lo += scr2[0])); ptrP >= P; *ptrP-- = sum) {
			ptr1 = ptrP - first;
			ptr1e = ptrP - last;
			ptr2 = p + first;
