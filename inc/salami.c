...
get_scores(void *space, struct pair_set *set, struct coord *a, struct coord *b, void *to_use) {
	int cover = 0, i;
	float cover_float;
	float score, smpl_score, scr_tot;
	char *pcover1;
	char *pcover2;
	struct seq* seq_a;
...
void latexWurstAlignment(void *space, Matchtype *m, IntSequence **s, int len, void *info) {
	char *p_a;
	char *bin;
	char *vec;
	float frac_dme, z_scr;
	...
	set = score_mat_sum_full(&crap, matrix, sw1_pgap_open, sw1_pgap_widen, sw1_pgap_open, sw1_pgap_widen,
	NULL, NULL, N_AND_W, NULL);
...
struct salami_info* alignment_wurst(void *config, void *space, Matchtype *match, IntSequence **sequences, int len, void *info) {
	float frac_dme, z_scr;
	float *altscores;
	unsigned int id;
	float mean, dev, rmsd, andrew_scr;
