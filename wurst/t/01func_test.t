...
$Coord=pdb_read ("extras/testfiles/1H1W.pdb", '', '');

if(!coord_2_bin($Coord, "extras/testfiles/1H1W.bin")){
 BAIL_OUT("coord_2_bin unable to write bin file");
}
$Coord2=coord_read ("extras/testfiles/1H1W.bin");


$classfcn = aa_strct_clssfcn_read("extras/testfiles/classfile", 0.4);
$pvecout = strct_2_prob_vec($Coord2, $classfcn);
if(!prob_vec_write($pvecout, "1H1W.bak")){
 BAIL_OUT("prob_vec error");
}

$pvec = prob_vec_read("1H1W.bak");
...
is ($cgg_values[3], 1, "coord_geo_gap num_gap");
...
is ($pss_values[0], 1680, "pair_set_score gap_score");
is ($pss_values[1], 1680, "pair_set_score no_gap_score");
...
is ($gr_values[1], 285, "get_rmsd count");