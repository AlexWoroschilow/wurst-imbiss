 ...
 * $Id: model.c,v 1.4 2012/02/15 13:36:05 ploeffler Exp $
 */
...
/* ---------------- constants  --------------------------------
 */
/* This is a limit on the number of times we print out the
 * warnging for dead models.
 */
#define MAX_ERR 50
            ...
            const struct seq *seq, struct coord *coord)
{
    ...
    if (max == 0) {
        static unsigned char err_cnt    = MAX_ERR;
        static unsigned char turned_off = 0;
        if (turned_off != 0) {
            err_printf (this_sub, "Returning empty model\n");
            if ( --err_cnt == 0) {
                turned_off = 1;
                err_printf (this_sub, "Turning off empty model warning.\n");
            }
        }
        return NULL;
    }
