...
Uint minvecdist(void *space, vector_t *vec, Uint i, Uint j) {
	Uint k, size_i, size_j;
	int range, dist = INT_MAX, l;
	vectorelem *e_i, *e_j;
...
Uint minvecdist2(void *space, vector_t *vec1, vector_t *vec2, Uint *which) {
	Uint k, size_i, size_j;
	int range, dist = INT_MAX, l;
	vectorelem *e_i, *e_j;
...
double uniroot(double start, double end, double (*f)(double, void*), double tolx, void* info) {
	double a, b, c;
	double fa, fb, fc;
	double prev;
	double currenttol;
	...
	while (1) {
		...
		if (fabs(prev) >= currenttol && fabs(fa) > fabs(fb)) {
			...
			if (p < (0.75 * cb * q - fabs(currenttol * q) / 2) && p < fabs(prev * q / 2)) {
				new_step = p / q;
			}
		}
...
double simpson(double a, double b, int sdiv, double (*f)(double, void*), void* info) {
...
double simpson1D(double x, int sdiv, double (*f)(double, double, void*), double (*c)(double, void*), double (*d)(double, void*), void* info) {

	double k, sum1 = 0, sum2 = 0, ca, da;
	int i;
...
double simpson2D(double a, double b, int sdiv, int sdiv2, double (*f)(double, double, void*), double (*c)(double, void*),
		double (*d)(double, void*), void *info) {
	...
	return ((double) (h / 3) * (simpson1D(a, sdiv2, f, c, d, info) + simpson1D(b, sdiv2, f, c, d, info) + 2 * sum1 + 4 * sum2));
}
...
double det(void *space, double *m, int n) {
	double sum = 0;
	double *t = NULL;
	int j;
...
double samplevar(double *x, double *p, double n) {
	int i;
	double m, r, sum = 0;
...
double stddev(double *x, double n) {
...
double bivarcond(double x, double y, double mu1, double mu2, double *cv) {
	double rho, s1, s1sq, s2, s2sq, m, e;
...
double bivarnorm(double x, double y, double mu1, double mu2, double* cv) {
	double rho, s1, s1sq, s2, s2sq, m, e1, e2;
	...
	e1 = (-1) / (2 * (1 - (rho * rho)));
	e2 = ((x - mu1) * (x - mu1)) / s1sq - (2 * rho * (x - mu1) * (y - mu2)) / (s1 * s2) + ((y - mu2) * (y - mu2)) / s2sq;
