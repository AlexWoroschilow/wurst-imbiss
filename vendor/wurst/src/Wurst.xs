#/* $Id: Wurst.xs,v 1.25 2015/09/24 12:31:33 wurst Exp $ */
...
typedef struct calpha_clssfcn   Ca_clssfcn;
...
typedef struct prob_vecs        Prob_vecs;
typedef struct sequences_list   Sequences_list;
...
Pair_set *
merge_localigns(align1, align2, c1, c2)
   Pair_set *align1
   Pair_set *align2
   int c1
   int c2;
...
MODULE = Wurst        PACKAGE = Prob_vecsPtr PREFIX = prob_vecs_

void
prob_vecs_DESTROY(ps)
    Prob_vecs *ps;
  CODE:
    prob_vecs_destroy(ps);
...
MODULE = Wurst        PACKAGE = Sequences_listPtr PREFIX = sequences_list_

void
sequences_list_DESTROY(ps)
    Sequences_list *ps;
   CODE:
     sequences_list_destroy(ps);
...
char *
printconserv(consv, len, pair_set, startpos, maxlen, strNum1)
   float *consv
   size_t len
   Pair_set *pair_set
   size_t startpos
...
char *
conserv_str(consv, len)
  float *consv
  size_t len;
      
char *
pair_set_get_strNum (ps, strNum, c)
    Pair_set *ps
    size_t strNum
    Coord *c;   
    
float*
get_seq_conserv(pair_set, ps);
   Pair_set *pair_set
   Sequences_list* ps;    
...
void 
bit_set_bitset (bs, pos)
    char *bs
    size_t  pos;

char
bit_set_ifbitset (bs, pos)
    char *bs
    size_t pos;

void
bit_set_clean (bs, pos)
    char *bs
    size_t pos;

#/* ---------------- Selection of active sites --------------------------- */

void
pair_set_sel_geti (align)
    Pair_set *align
    
    PPCODE:
      char *res = pair_set_sel_geti(align); 
      SV *out = sv_2mortal(newSVpv(res, bit_set_getsize(align->n)));
      bit_set_free(res);
      XPUSHs(out);
...
void 
pair_set_print_prepare (s1, s2, align, pss)
    Seq *s1
    Seq *s2
    Pair_set *align
    char *pss;
...
Pair_set *
selected_pair_set_get (source, pss)
    Pair_set *source
    char *pss;

char *
pair_set_sel_print(pss, bs_len)
    char *pss
    size_t bs_len
    PPCODE:
        char *res = pair_set_sel_print(pss, bs_len); 
        SV *out = sv_2mortal(newSVpv(res, bs_len));
        bit_set_free(res);
        XPUSHs(out);
   
        
size_t 
pair_set_sel_len(align)
    Pair_set * align

char *
pair_set_sel_delmaxdistance (c1, c2, align, source)
    Coord *c1
    Coord *c2
    Pair_set *align
    char *source
    PPCODE:
      RETVAL = pair_set_sel_delmaxdistance(c1, c2, align, source);
      PUSHs(sv_2mortal(newSVpv(RETVAL, bit_set_getsize(align->n))));
...
float
coord_ca_ca_dist (c, i, j, sqrt_flag)
        Coord *c
        unsigned int i
        unsigned int j
        unsigned int sqrt_flag;
      CODE:
        RETVAL = coord_ca_ca_dist (c, i, j, sqrt_flag);
      OUTPUT:
        RETVAL
...
float
coord_omega (c, i, shift_min);
        Coord *c
        const size_t i
        const float shift_min
      CODE:
        RETVAL = coord_omega (c, i, shift_min);
        if (RETVAL == BAD_ANGLE_FLOAT)
            XSRETURN_UNDEF;
      OUTPUT:
        RETVAL

float
coord_tau (c, i, shift_min);
        Coord *c
        const size_t i
        const float shift_min
      CODE:
        RETVAL = coord_tau (c, i, shift_min);
        if (RETVAL == BAD_ANGLE_FLOAT)
            XSRETURN_UNDEF;
      OUTPUT:
        RETVAL
...
float
coord_theta (c, i, shift_min);
        Coord *c
        const size_t i
        const float shift_min
      CODE:
        RETVAL = coord_theta (c, i, shift_min);
        if (RETVAL == BAD_ANGLE_FLOAT)
            XSRETURN_UNDEF;
      OUTPUT:
        RETVAL
...
void
score_mat_remove_alignment( smat, p_s )
         Score_mat *smat
         Pair_set  *p_s;
...
char *
pair_set_stringI(pairSetString, pos, seq)
        Pair_set *pairSetString
        size_t pos
        Seq *seq;

char *
pair_set_stringN(pairSetString, pos)
        Pair_set *pairSetString
        size_t pos;
...
char *
pair_set_fasta_string (s, seq0, seq1)
        Pair_set *s
        Seq *seq0
        Seq *seq1;

size_t 
pair_set_get_startpos(s, pos)
      Pair_set *s
      size_t pos;
...
int
pair_set_get_alignmentlength( p_s )
        Pair_set *p_s;

int
pair_set_get_netto_alignmentlength( p_s )
        Pair_set *p_s;
...
struct triplet
get_seq_id ( Pair_set *pair_set, Seq *s1, Seq *s2 )
    PROTOTYPE: $$$
    INIT:
        struct triplet t;
    PPCODE:
        t = get_seq_id(pair_set, s1, s2);
        XPUSHs(sv_2mortal(newSVnv(t.length)));
        XPUSHs(sv_2mortal(newSVnv(t.aligned)));
        XPUSHs(sv_2mortal(newSVnv(t.ident)));
...
int
pair_set_write_traceback_to_file( p_s, s1, s2, fname, type )
		Pair_set *p_s
		Seq *s1
		Seq *s2
		char *fname
...
int
score_pvec_gl (score_mat, pvec1, pvec2)
    Score_mat *score_mat
    Prob_vec *pvec1
    Prob_vec *pvec2
    CODE:
        RETVAL = score_pvec_gl (score_mat, pvec1, pvec2);
        if (RETVAL == EXIT_FAILURE)
            XSRETURN_UNDEF;
        else
            RETVAL = 1;
...
Prob_vec *
prob_vec_rearrange( pvec, rearr_index )
    Prob_vec *pvec
    int rearr_index;
...
float *
get_focf(struct_list, size, query_vec, bin_fpc)
    char * struct_list
    int size
    Prob_vec *query_vec
    char * bin_fpc
...
float *
get_threshs(focf, num_focf, first_perc, num)
    float * focf
    int num_focf
    int first_perc
    int num
...
float
get_float_value(values, index)
    float * values
    int index

int
write_fpc_bin(vec_lib, target)
    char * vec_lib
    char * target
...
#/* ---------------- Calpha classification ---------------------------
# */

MODULE = Wurst  PACKAGE = Wurst

Ca_clssfcn *
ac_read_calpha (fname, t_error, c_error, corr_num)
        const char *fname
        float t_error
        float c_error
        int corr_num
...
float
calpha_clssfcn_return (ca,i_class,pos,att_num)
    Ca_clssfcn *ca
    int i_class
    int pos
    int att_num
...
float
ac_class_wt_calpha (ca,i_class)
    Ca_clssfcn *ca
    int i_class

Prob_vec *
calpha_strct_2_prob_vec (structure, ca, norm = 1)
    Coord * structure
    Ca_clssfcn * ca
    int norm

void
calpha_clssfcn_destroy (ca)
        Ca_clssfcn * ca
    CODE:
        calpha_clssfcn_destroy (ca);

MODULE = Wurst   PACKAGE = Ca_clssfcnPtr PREFIX = Ca_clssfcn_
...
Pair_set *
merge_align (align1, align2, alignment);
        Pair_set * align1
        Pair_set * align2
        Pair_set * alignment;
...
Pair_set *
merge_alignments (align1, align2, alignment, c1, c2);
        Pair_set * align1
        Pair_set * align2
        Pair_set * alignment
		int c1
...
Prob_vec *
pvec_avg (p_vec1, p_vec2, p_set, weight);
        Prob_vec * p_vec1
        Prob_vec * p_vec2
		Pair_set * p_set
		float weight;
...
double
tm_score(pairset, r1, r2); 
	Pair_set * pairset
	Coord * r1
	Coord * r2;
#/* ---------------------- TM-Score_s --------------------------------
# *        same as TM-Score, but normalizing over the shorter struct
# */

double
tm_score_s(pairset, r1, r2); 
	Pair_set * pairset
	Coord * r1
	Coord * r2;


#/* ---------------------- apdb scoring function ---------------------
#*/

float
get_apdb(pairset, r1, r2); 
	Pair_set * pairset
	Coord * r1
	Coord * r2;
