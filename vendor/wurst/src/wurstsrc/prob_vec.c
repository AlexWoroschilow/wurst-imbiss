 ...
 * $Id: prob_vec.c,v 1.4 2014/07/20 12:18:37 torda Exp $
 */
...
prob_vec_read (const char *fname)
{
    ...
    file_clear_cache (fp); /* calls posix function to free cached data */
    fclose (fp);
    return p_v;
 error:
    if (p_v)
...
/* ---------------- prob_vec_fragmentlength    --------------------------
 * returns the length of the fragments depending on the chosen classmodel.
 */
size_t
prob_vec_fragmentlength(const struct prob_vec *pvec){
    return  pvec->frag_len;
}
