/*
 * 27 Aug 2001
 * $Id: align.c,v 1.12 2015/08/03 12:54:27 torda Exp $
 */
                     ...
                     const struct coord *c)
{
    ...
    const char *this_sub = "pair_set_get_strNum";
    int **indices;
    size_t count = 0;
    size_t i;
    ...
    scr_reset();
    for (i = 0; i < ps->n; i++) {
        if (indices[i][strNum] != GAP_INDEX) {
            if (count > 0)
                scr_printf (", %d", c -> orig[indices[i][strNum]]);
            else
                scr_printf ("%d", c -> orig[indices[i][strNum]]);
            count++;
        }
                    ...
                    const int algn_type, const struct pair_set *bias_set)
{
    ...
    const char *too_many = "Error message printed %u times. Not printing any more\n";
    const char *this_sub = "sum_score_mat";
    const unsigned short MAX_ERR_WID = 25;
    ...
    if ((p_widen > p_open) || (q_widen > q_open)) {
        ...
        if (n_printed <= MAX_ERR_WID) {
            err_printf (this_sub, gap_err, wid, p_open, p_widen, q_open, q_widen);
            n_printed++;
        }
        if (n_printed == MAX_ERR_WID)
            err_printf (this_sub, too_many, n_printed);
    }
    if (bias_set)
    ...
    if (row_mult_orig || col_mult_orig) {
        ...
        no_fancy_shit = 1;/* true - no fancy shit */
    }
