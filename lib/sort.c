...
Uint cmp_dbl(Uint a, Uint x, void *data, void *info) {
	 ...
	 */
	if (fabs((double) d[a] - d[x]) <= FLT_EPSILON)
		return 0;
	if ((double) d[a] - d[x] > FLT_EPSILON)
		return 1;
	if ((double) d[x] - d[a] > FLT_EPSILON)
		return 2;
...
Uint binarySearch_m(void *toSearch, Uint size, void *key, Uint (*cmp)(Uint, void *, void *, void *), void *info) {
	int left = 0, right = size, mid = 0, res = 0;
...
Uint binarySearch(void *toSearch, Uint size, void *key, Uint (*cmp)(Uint, void *, void *, void *), void *info) {
...
Uint *quickSort(void *space, void* toSort, Uint size, Uint (*cmp)(Uint, Uint, void *, void*), void *info) {
...
Uint compareMkstrptr(Uint a, Uint b, Uint depth, void *data, void* info) {
	char **ptr = (char**) data;

	if (ptr[a][depth] > ptr[b][depth])
		return 1;
	if (ptr[a][depth] < ptr[b][depth])
		return 2;
...
Uint compareMkstr(Uint a, Uint b, Uint depth, void *data, void* info) {
	char *ptr = (char*) data;
...
void vecswap(int i, int j, int n, Uint *x) {
	while (n-- > 0) {
		SWAPUINT(x, i, j);
		i++;
		j++;
...
Uint *
quickSortMultikey(void *space, void* toSort, Uint size, Uint (*cmp)(Uint, Uint, Uint, void *, void*), Uint sentinel, void *info) {
	Sint a, b, c, d, v, n, r;
...
int cmp_PairUint_qsort(const void* a, const void* b) {
	PairUint *i;
	PairUint *j;
...
int cmp_PairUint_bsearch(const void* a, const void* b) {
	Uint *key;
	PairUint *obj;
...
int cmp_PairSint_qsort(const void* a, const void* b) {
	PairSint *i;
	PairSint *j;
...
int cmp_PairSint_bsearch(const void* a, const void* b) {
	int *key;
	PairSint *obj;
